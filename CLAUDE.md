...
这套“华为云远程”指华为云 ModelArts 训练作业（Training Job）容器，不是 ECS / 弹性云服务器：

- 对外 SSH 入口是 `dev-modelarts-cnnorth9.huaweicloud.com:<port>`，端口由 ModelArts 为 Running 作业动态映射；`30000` 段端口不是 ECS 实例 ID，也不是稳定主机编号
- 登录用户通常是 `ma-user`，进入的是训练作业的 `worker-0` 容器；底层由 ModelArts 资源池调度到 Kubernetes Pod / 物理 GPU 节点
- `stable` / `preemptible` 是远程资源或作业的可用性标签；preemptible 容器可能被平台回收，不能按长期 VM 运维
- 本项目的 `jobs` / `detail` / `usage` / `exec` / `copy` / `stop` / `delete` 都围绕 ModelArts 控制台 `training-jobs` REST API 工作
